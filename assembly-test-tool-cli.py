...
fpath        = "/home/pi/Desktop/assembly_csv_files/"
fname        = fpath # append with name of device
foutpath     = "/home/pi/Desktop/assembly_event_outputs/"
        ...
        for dev in devs:
            if dev.getValueText(9) is not None:
                if sensor_name in dev.getValueText(9):
                    ...
                    print("*** Press 'd' on the keyboard to disconnect from Lura device\n")
                    write_csv_header()
...
def disconnect_from_device():
    ...
    tx_char_list = sensor_obj.getCharacteristics(uuid=tx_uuid)
    tx_char = tx_char_list[0]
    tx_char.write("PWROFF".encode('utf-8'), False)
...
print("\n* * * * * * * * * * * * * * * * * * * * * * * * * * *")
print("*        Beginning Lura  Assembly Testing Tool      *")
print("* * * * * * * * * * * * * * * * * * * * * * * * * * *")
print(" ")
print("*** Press 'Ctrl + C' or click the 'x' button to close this tool")
print(" ")
print("*** BLE Scanning for Lura device...")
print(" ")
print("          If no connection occurs within 5 seconds of pressing") 
print("          the button, there is an issue with the PCB.\n") 
...
while True:
    try:
        ...
        if sensor_obj.waitForNotifications(3.0):
            pass
        # If user enters 'd', fire the disconnect event 
        if (kb.kbhit()):
            c = kb.getch()
            if ord(c) == 100:
        ...
        if "Failed" in str(e):
            try:
                pixels.fill((0,0,0))
                print("!!! Whoops, there was a problem in the application !!!\n!!! Check /home/pi/Desktop/assembly_testing_event_outputs for more info \n")
                t.sleep(1)
                pixels[ERR] = RED
                sys.exit(0)
            except SystemExit:
